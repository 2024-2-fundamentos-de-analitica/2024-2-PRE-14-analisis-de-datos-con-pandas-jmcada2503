...
# (columna DepDelay).
delayedFlights = flights[flights["DepDelay"]
                         > 15][["UniqueCarrier", "DepDelay"]]
...
flights["IsDelayed"] = flights["IsDelayed"].map(
    lambda x: 0 if pd.isna(x) else x)
...
flights["hour"] = flights["hour"].map(
    lambda x: int(x / 100) if not pd.isna(x) else x)
